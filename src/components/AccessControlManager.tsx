...
export const AccessControlManager: React.FC<AccessControlManagerProps> = ({
  ...
  const [formRole, setFormRole] = useState<AdminRole>('super_admin');
  ...
  // Cloud Synchronization and Async Operation States
  ...
  const handleManualSync = async () => {
    ...
    setFormError(null);
    try {
      const cloudUsers = await getAdminUsersFromFirestore();
      if (cloudUsers.length > 0) {
        setAdminUsers(cloudUsers);
        localStorage.setItem('mbti_industrial_admin_users_v1', JSON.stringify(cloudUsers));
        setSyncSuccessMessage(`Sinkronisasi sukses! ${cloudUsers.length} akun petugas terverifikasi dari Cloud Firestore.`);
      } else {
        setSyncSuccessMessage('Terhubung dengan Cloud Firestore. Seluruh akun petugas tersinkronkan.');
      }
      setTimeout(() => setSyncSuccessMessage(null), 4000);
    } catch (err: any) {
      setFormError('Gagal menyinkronkan data dari cloud: ' + (err.message || err));
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        const result = await saveAdminUserAsync({
          id: editingUser.id,
        ...
        setSyncSuccessMessage(`Perubahan akses ${emailTrimmed} berhasil disimpan dan disinkronkan ke seluruh PC.`);
      } else {
        const result = await saveAdminUserAsync({
          fullName: formName.trim(),
          email: emailTrimmed,
        ...
        setSyncSuccessMessage(`Petugas baru ${emailTrimmed} berhasil ditambahkan dan disinkronkan ke Cloud Firestore.`);
      }
      ...
      setFormError('Terjadi kesalahan saat menyimpan akun: ' + (err.message || err));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className={`text-xs mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
              Hanya email yang terdaftar pada tabel otorisasi di bawah yang diizinkan mengakses Dashboard Admin & Data Psikometri di seluruh komputer/PC.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          ...
          {canManage ? (
            ...
            <button
              disabled
              id="add-admin-user-button-disabled"
              className={`px-4 py-2 rounded-lg text-xs font-label-caps font-semibold uppercase tracking-wider flex items-center gap-2 border cursor-not-allowed shrink-0 ${
                isDarkMode ? 'bg-slate-800/50 border-slate-700 text-slate-500' : 'bg-slate-100 border-slate-300 text-slate-400'
              }`}
              title="Akun Anda tidak memiliki wewenang mengelola akun petugas. Hanya Super Administrator dan HRD yang dapat menambahkan petugas."
            >
              <Lock className="w-4 h-4 text-amber-400" />
              <span>Tambah Akses Petugas (Terkunci)</span>
            </button>
          )}
        </div>
      </div>

      {/* Notice if logged in user does NOT have permission */}
      {!canManage && (
        <div className={`p-3.5 rounded-xl border flex items-center gap-3 text-xs ${
          ...
          <div>
            <span className="font-semibold">Perhatian Akses Terbatas:</span> Anda saat ini login sebagai{' '}
            <strong className="underline">{currentAdmin.roleLabel || currentAdmin.role}</strong>. Peran ini berada dalam mode peninjau (Read-Only) pada manajemen hak akses. Untuk menambahkan atau mengedit petugas baru di PC ini, silakan keluar dan masuk menggunakan akun <strong>Super Administrator</strong> atau <strong>HRD</strong>.
          </div>
        </div>
      )}

      {/* Toast Alert for Cloud Sync Success */}
      {syncSuccessMessage && (
        <div className="p-3.5 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs flex items-center justify-between gap-3 shadow-sm">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
            <span className="font-medium">{syncSuccessMessage}</span>
          </div>
          <button onClick={() => setSyncSuccessMessage(null)} className="text-emerald-400 hover:text-emerald-200 p-0.5">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            ...
            >
              ...
              <form onSubmit={handleSaveUser} className="space-y-4 text-xs">
                ...
                {/* Role description and authority preview */}
                <div className={`p-3 rounded-lg border text-[11px] space-y-1 ${
                  ...
                  <div className="flex items-center gap-1.5 pt-1">
                    {ROLE_DEFINITIONS[formRole].permissions.canManageAdmins ? (
                      <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 font-semibold">
                        <Check className="w-3 h-3" /> Berwenang Menambahkan & Mengelola Petugas Lain (Di Semua PC)
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded bg-amber-500/15 text-amber-400 border border-amber-500/30 font-semibold">
                        <Lock className="w-3 h-3" /> Akses Operasional (Tidak Dapat Menambahkan Petugas Lain)
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-800">
                  <button
                    ...
                    disabled={isSaving}
                    onClick={() => setIsModalOpen(false)}
                    className="px-3.5 py-2 rounded-lg border border-slate-700 text-slate-400 hover:text-white transition-colors disabled:opacity-50"
                  >
                    Batal
                  </button>
                  <button
                    type="submit"
                    disabled={isSaving}
                    className="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white font-semibold flex items-center gap-1.5 transition-all shadow-md active:scale-95"
                  >
                    {isSaving ? (
                      <>
